...
#include "BeJwrMSo2zUlMSs3JKTEuNctLrSjxdEkySi9xMy1LzClNNS0qzUktBgABDg3A.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring label,list<tuple<uint32 nextID,boolean gt,float64 value>> rules>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("rules");
        TypeOffset t(MY_OFFSETOF(rules_, tm), 
                     Meta::Type::typeOf<SPL::list<SPL::BeJwrMS41y0utKPF0STJKL3EzLUvMKU0FAFLpAem > >(), 
                     &typeid(SPL::list<SPL::BeJwrMS41y0utKPF0STJKL3EzLUvMKU0FAFLpAem >));
        addMapping(*tm, t, s, 1);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "rules") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, rules_);
   else
     istr >> rules_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, rules_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "rules=", get_rules()) ;
    ostr << '}';
}
...
SELF& SELF::clear()
{
    ...
    if (get_rules().size() < 1024) {
        get_rules().clear();
    }
    else {
        SPL::list<SPL::BeJwrMS41y0utKPF0STJKL3EzLUvMKU0FAFLpAem >().swap(get_rules());
    }
