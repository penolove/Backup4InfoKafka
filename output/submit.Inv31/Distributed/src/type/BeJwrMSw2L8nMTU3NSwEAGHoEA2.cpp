...
#include "BeJwrMSw2L8nMTU3NSwEAGHoEA2.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring timeend>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("timeend");
        TypeOffset t(MY_OFFSETOF(timeend_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "timeend") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, timeend_);
   else
     istr >> timeend_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, timeend_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "timeend=", get_timeend()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_timeend());
