...
#ifndef BEJWRMSW2L8NMTU3NSWEAGHOEA2_H_
#define BEJWRMSW2L8NMTU3NSWEAGHOEA2_H_
...
#define SELF BeJwrMSw2L8nMTU3NSwEAGHoEA2
...
public:
    ...
    typedef SPL::rstring timeend_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), timeend_() {}
    SELF(const Self & ot) : Tuple(), timeend_(ot.timeend_) 
      { constructPayload(ot); }
    SELF(const timeend_type & _timeend) : Tuple(), timeend_(_timeend) { }
    ...
    timeend_type & get_timeend() { return timeend_; }
    const timeend_type & get_timeend() const { return timeend_; }
    void set_timeend(const timeend_type & _timeend) { timeend_ = _timeend; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<timeend_type >()(timeend_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += timeend_.getSerializedSize();
...
#undef SELF
#endif // BEJWRMSW2L8NMTU3NSWEAGHOEA2_H_ 
