...
#include "BeJwrMcw0TSnNza0EABAkANw.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<int32 dummy>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("dummy");
        TypeOffset t(MY_OFFSETOF(dummy_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "dummy") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, dummy_);
   else
     istr >> dummy_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, dummy_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "dummy=", get_dummy()) ;
    ostr << '}';
}
