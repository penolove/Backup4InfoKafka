...
#include "BeJwrMSw2ycnMSwUADOECD1.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring line>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("line");
        TypeOffset t(MY_OFFSETOF(line_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "line") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, line_);
   else
     istr >> line_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, line_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "line=", get_line()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_line());
