...
#include "BeJwrMS4xKTYvLkp2TEkp8gYxAvKLSorNU4pLICJABkjELC0nv9zTpdg0JzEpNcfNPCexJDXP0wUA62wXBP.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<tuple<rstring srcAddr,uint16 srcPort,rstring dstAddr,uint16 dstPort> flowID,rstring label,float64 latenID>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("latenID");
        TypeOffset t(MY_OFFSETOF(latenID_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "latenID") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, latenID_);
   else
     istr >> latenID_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, latenID_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "latenID=", get_latenID()) ;
    ostr << '}';
}
